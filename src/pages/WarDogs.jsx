...
export default function WarDogs(){
  ...
  return <>
    ...
    <main className="article-layout wrap"><article className="article-body"><p className="article-lead">{a.lead}</p><p>{a.intro}</p><h2>{a.h2a}</h2><p>{a.p1}</p><div className="article-pullquote"><span>“</span><p>{a.quote}</p><small>{a.quoteBy}</small></div><p>{a.p2}</p><h2>{a.h2b}</h2><p>{a.p3}</p><p>{a.p4}</p><div className="article-endnote"><span className="live-dot" /><strong>{a.take}</strong><span>{a.takeText}</span></div><p className="article-source">{a.source} <a href="https://news.instant-gaming.com/fr/articles/21838-wardogs-sortira-sur-consoles-en-2028" target="_blank" rel="noreferrer">{a.sourceDetail}</a></p></article><aside className="article-aside"><div className="aside-card"><span className="aside-kicker">{a.quick}</span><strong>{a.switch}</strong><strong>{a.stakes}</strong><strong>{a.unknowns}</strong></div><div className="aside-card aside-card-accent"><span className="aside-kicker">{a.next}</span><strong>{a.nextTitle}</strong><p>{a.nextText}</p><Link className="arrow-link" to="/news">{a.back} <Arrow/></Link></div></aside></main>
