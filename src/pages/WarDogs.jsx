...
export default function WarDogs(){
  const { t } = useLanguage();
  const a = t.news.wardogs;
  const gallery = getArticleGallery('wardogs');
  return <>
    <section className="article-hero wrap"><div className="section-label"><span><b>{a.label.split(' / ')[0]}</b> / {a.label.split(' / ')[1]}</span><span>{a.date}</span></div><div className="article-heading"><div><p className="eyebrow"><span className="live-dot" /> {a.eyebrow}</p><h1>{a.title}<br/><em>{a.titleAccent}</em></h1><p className="article-dek">{a.dek}</p><div className="article-byline"><span>{a.byline}</span><span>{a.readTime}</span></div></div><div className="article-cover hud-frame"><img src={`${base}wardogs-news.jpg`} alt={a.coverAlt} /><div><small>{a.coverKicker}</small><strong>WARDOGS</strong></div></div></div></section>
    <main className="article-layout wrap"><article className="article-body"><p className="article-lead">{a.lead}</p><p>{a.intro}</p><h2>{a.h2a}</h2><p>{a.p1}</p>{gallery ? <ArticleGallery {...gallery} /> : null}<div className="article-pullquote"><span>“</span><p>{a.quote}</p><small>{a.quoteBy}</small></div><p>{a.p2}</p><h2>{a.h2b}</h2><p>{a.p3}</p><p>{a.p4}</p><div className="article-endnote"><span className="live-dot" /><strong>{a.take}</strong><span>{a.takeText}</span></div><p className="article-source">{a.source} <a href="https://news.instant-gaming.com/fr/articles/21838-wardogs-sortira-sur-consoles-en-2028" target="_blank" rel="noreferrer">{a.sourceDetail}</a></p></article><aside className="article-aside"><div className="aside-card aside-card-accent"><span className="aside-kicker">{a.next}</span><strong>{a.nextTitle}</strong><p>{a.nextText}</p><Link className="arrow-link" to="/news">{a.back} <Arrow/></Link></div></aside></main>
    <Comments />
